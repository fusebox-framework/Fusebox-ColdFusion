...
<cfcomponent output="false" hint="I represent a circuit.">
	<cfset  circuitImplicit = false />
	
	<cffunction name="init" intent:returntype="fuseboxCircuit" access="public" output="false" 
		...
		<cfif circuitImplicit>
			<cfreturn createObject("component", "fuseboxNoXMLCircuit").init(arguments.fbApp, arguments.alias, arguments.myFusebox, arguments.path) />
		<cfelse>
			<cfreturn this />
		</cfif>
	</cffunction>
	...
	<cffunction name="reload" returntype="fuseboxCircuit" access="public" output="false" 
		...
		<cfset var circuitFile = "" />
		<cfset var candidateCircuitFiles = "circuit.xml.cfm,circuit.xml,#variables.alias#.xml.cfm,#variables.alias#.xml" />
		<cfset var storedCircuitPath = "" />
		<cfset var circuitFound = false />
		...
		--->
		<cfloop list="#candidateCircuitFiles#" index="circuitFile">
			<cfif fileExists(variables.fullPath & circuitFile)>
				<cfif NOT variables.fuseboxApplication.strictMode>
					<cfif variables.fuseboxApplication.debug>
						<cfset arguments.myFusebox.trace("Compiler","Circuit Found at: " & variables.fullPath & circuitFile) />
					</cfif>
					<cfbreak />
				<cfelseif NOT circuitFound>
					<cfset storedCircuitPath = circuitFile />
					<cfset circuitFound = true />
				<cfelse>
					<cfthrow type="fusebox.multipleCircuitXML" 
							message="Multiple candidate circuits found, this is disallowed in strict mode." 
							detail="'#storedCircuitPath# and #circuitFile# found in '#variables.fullPath#." />
				</cfif>
			</cfif>
		</cfloop>
		<cfif variables.fuseboxApplication.strictMode>
			<cfset circuitFile = storedCircuitPath />
		</cfif>
		...
		<cfif needToLoad>
			...
			<cftry>
				...
				<cfset circuitXML = variables.fullPath & circuitFile />
