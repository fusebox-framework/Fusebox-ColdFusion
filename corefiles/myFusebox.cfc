...
<cfcomponent hint="I provide the per-request myFusebox data structure and some convenience methods.">
	...
	<cffunction name="getOriginalFuseaction" returntype="any" access="public" output="false" 
		...
		<cfreturn getOriginalCircuit().fuseactions[this.originalFuseaction] />
	...
	<cffunction name="do" returntype="any" access="public" output="true" 
		...
		<cfset var output = "" />
		<cfif (arguments.returnOutput or arguments.contentVariable is not "") AND arguments.passThroughReturn>
			<cfthrow errorcode="Fusebox.Missmatch_Parameters" message="Paramters are in conflict." detail="Chances are you went and asked Fusebox to output the return as well as pass it back through. I think that's sort of silly and outright dangerous so I made this little error up for you.">
		</cfif>
		<cfset output =	getApplication().do(
					arguments.action,
					this,
					arguments.returnOutput or arguments.contentVariable is not "",
					arguments.passThroughReturn) />
