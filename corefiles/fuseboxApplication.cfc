...
<cfcomponent output="false" hint="I am the Fusebox application object, formerly the application.fusebox data structure.">
	...
	<cffunction name="init" returntype="fuseboxApplication" access="public" output="false" 
		...
		<cfset variables.pathUtil = createObject("component", "PathUtil").init(arguments.appPath, arguments.callerPath) />
		<cfset structAppend(this,pathUtil.compatibility(),true) />
		...
		<cfif needToLoad>
			...
			<cfif fbFileExists>
				<cftry>
					...
					<cfset fbXML = this.approotdirectory & fbFile  />
			...
			--->
			<cfset this.parsePath = pathUtil.normalizePartialPath(this.parsePath) />
			<cfset this.parseRootPath = pathUtil.relativePath(expandFuseboxPath(this.parsePath),getApplicationRoot()) />
			<cfset this.pluginsPath = pathUtil.normalizePartialPath(this.pluginsPath) />
			<cfset this.lexiconPath = pathUtil.normalizePartialPath(this.lexiconPath) />
			<cfset this.errortemplatesPath = pathUtil.normalizePartialPath(this.errortemplatesPath) />
	...
	<cffunction name="do" returntype="any" access="public" output="true" 
		...
		<cfinvoke component="fuseboxExecutionContext" method="__executeDynamicDo" returnvariable="output"
					parsedFileInfo="#parsedFileInfo#" myFusebox="#arguments.myFusebox#" returnOutput="#arguments.returnOutput#" passThroughReturn="#arguments.passThroughReturn#"/>
	...
	<cffunction name="loadCircuits" returntype="void" access="private" output="false" 
		...
		<cfset var definedCircuits = xmlSearch(arguments.fbCode,"/fusebox/circuits/circuit") />
		<cfset var i = 0 />
		<cfset var n = arrayLen(definedCircuits) />
		...
		<cfloop from="1" to="#n#" index="i">
			<cfif not structKeyExists(definedCircuits[i].xmlAttributes,"alias")>
			...
			<cfif not structKeyExists(definedCircuits[i].xmlAttributes,"path")>
				<cfthrow type="fusebox.badGrammar.requiredAttributeMissing"
						message="Required attribute is missing"
						detail="The attribute 'path' is required, for the declaration of circuit '#definedCircuits[i].xmlAttributes.alias#' in fusebox.xml." />
			</cfif>
			<cfif structKeyExists(definedCircuits[i].xmlAttributes,"parent")>
				<cfset parent = definedCircuits[i].xmlAttributes.parent />
				<cfset nAttrs = 3 />
			<cfelse>
				<cfset parent = "" />
				<cfset nAttrs = 2 />
			</cfif>
			<cfif structKeyExists(definedCircuits[i].xmlAttributes,"relative")>
				<cfif listFind("true,false,yes,no",definedCircuits[i].xmlAttributes.relative) eq 0>
					<cfthrow type="fusebox.badGrammar.invalidAttributeValue"
							message="Attribute has invalid value" 
							detail="The attribute 'relative' must either be ""true"" or ""false"", for the declaration of circuit '#definedCircuits[i].xmlAttributes.alias#' in fusebox.xml." />
				</cfif>
				<cfset relative = definedCircuits[i].xmlAttributes.relative />
				<cfset nAttrs = nAttrs + 1 />
			<cfelse>
				<cfset relative = true />
			</cfif>
			<cfif structKeyExists(definedCircuits[i].xmlAttributes,"access")>
				<cfif listFind("public,internal",definedCircuits[i].xmlAttributes.access) eq 0>
					<cfthrow type="fusebox.badGrammar.invalidAttributeValue"
							message="Attribute has invalid value" 
							detail="The attribute 'access' must either be ""public"" or ""internal"", for the declaration of circuit '#definedCircuits[i].xmlAttributes.alias#' in fusebox.xml." />
				</cfif>
				<cfset access = definedCircuits[i].xmlAttributes.relative />
				<cfset nAttrs = nAttrs + 1 />
			<cfelse>
				<cfset access = "" />
			</cfif>
			<cfif this.strictMode and nAttrs neq structCount(definedCircuits[i].xmlAttributes)>
				<cfthrow type="fusebox.badGrammar.unexpectedAttributes"
						message="Unexpected attributes"
						detail="Attributes other than 'alias', 'path', 'access', and 'parent' were found for the declaration of circuit '#definedCircuits[i].xmlAttributes.alias#' in fusebox.xml." />
			</cfif>
			<cfset alias = definedCircuits[i].xmlAttributes.alias />
			...
			<cfif structKeyExists(previousCircuits,alias) and
					definedCircuits[i].xmlAttributes.path is previousCircuits[alias].getOriginalPath() and
				...
				<cfset arguments.myFusebox.trace("Fusebox", "Reloading Circuit: #alias#")>
			...
			<cfelse>
				...
				<cfset this.circuits[alias] =
						createObject("component","fuseboxCircuit")
							.init(this,alias,definedCircuits[i].xmlAttributes.path,parent,arguments.myFusebox,relative) />
			</cfif>
		</cfloop>
	...
	<cffunction name="loadLexicons" returntype="void" access="private" output="false" 
		...
		<cfloop from="1" to="#n#" index="i">
			...
			<cfset aLex.path = pathUtil.normalizePartialPath(children[i].xmlAttributes.path) />
		...
		<cfloop collection="#attributes#" item="attr">
			<cfif len(attr) gt 6 and left(attr,6) is "xmlns:">
				...
				<cfset attributes[attr] = pathUtil.normalizePartialPath(attributes[attr]) />
	...
	<cffunction name="getExtensionManager" returntype="any" access="public" output="false">
			<cfreturn variables.extensionManager />		
	</cffunction>
	
	<cffunction name="loadExtensions" returntype="void" access="private" output="false" 
				hint="I load extensions to the Framework.">
		<cfargument name="fbCode" type="any" required="true" 
					hint="I am the parsed XML representation of the fusebox.xml file." />
		<cfset variables.extensionManager = createObject("component", "fuseboxExtensionManager").init(pathUtil, fbCode) />		
		<cfset variables.extensionManager.loadExtensions(pathUtil.getCorePath() & "extension/") />
	</cffunction>
	...
	<cffunction name="relativePath" intent:depricated="true" returntype="string" access="public" output="false" 
				hint="Passthrough to PathUtil ">
		...
		<cfreturn pathUtil.relativePath(arguments.from,arguments.to) />
	...
	<cffunction name="normalizePartialPath" returntype="string" access="public" output="false" 
		...
		<cfreturn pathUtil.normalizePartialPath(arguments.partialPath) />
	...
	<cffunction name="locateCfc" returntype="string" access="public" output="false"
		...
		<cfreturn pathUtil.locateCfc(filename) />
