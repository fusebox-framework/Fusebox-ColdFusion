...
--->
	...
	<cffunction name="onRequestStart" output="false">
		...
		<cfif variables.myFusebox.parameters.load>
			<cflock name="#application.ApplicationName#_fusebox_#variables.FUSEBOX_APPLICATION_KEY#" type="exclusive" timeout="300">
				...
				<cfelse>
					...
					<cfif _fba.precedenceFormOrURL is "URL" and isDefined("URL")>
		...
		<cfset myFusebox.getApplication().getExtensionManager().runRequestStartExtentions(attributes, event, myFusebox) />
		<cfif right(arguments.targetPage,4) is ".cfc">
			<cfset fbContext = createObject("component","FuseboxContext").init() />
			<cfset fbContext.setMyFusebox(myFusebox) />
			<cfset fbContext.setFuseboxEvent(event) />
		</cfif>
	</cffunction>
