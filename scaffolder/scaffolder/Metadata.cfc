<cfcomponent displayname="metadata.cfc" extends="utilities"
	hint="I introspect the DBMS and write/update the metadata XML. I read the metadata XML to provide the data to the code generator.">
<!---
...
--->
	...
	<cfproperty name="lIgnoreOnInsert" type="string" hint="I am the list of field names that must be ignored on insert." />
	<cfproperty name="lIgnoreOnUpdate" type="string" hint="I am the list of field names that must be ignored on update." />
	<cfproperty name="activeIndicator" type="string" hint="I am the name of a field that shows the record is active (not deleted)." />
	<cfproperty name="deletedIndicator" type="string" hint="I am the name of a field that shows the record is deleted." />
	...
	<cfproperty name="OSdelimiter" type="string" hint="The operating system directory separator." />
	...
	<cffunction name="init" returntype="metadata" output="No" 
		...
		<cfargument name="template" required="No" type="string" hint="I am a list of the chosen templates." />
		...
		<cfargument name="lIgnoreOnInsert" required="No" type="string" default="" hint="I am the list of field names that must be ignored on insert." />
		<cfargument name="lIgnoreOnUpdate" required="No" type="string" default="" hint="I am the list of field names that must be ignored on update." />
		<cfargument name="activeIndicator" required="No" type="string" default="" hint="I am the name of a field that shows the record is active (not deleted)." />
		<cfargument name="deletedIndicator" required="No" type="string" default="" hint="I am the name of a field that shows the record is deleted." />
		...
		<!--- Set controllertemplate, modeltemplate, viewtemplate --->
		<cfif isDefined("arguments.template")>
			<cfloop list="#arguments.template#" index="thisTemplate">
				<cfset setvariable("#listFirst(thisTemplate,"/\")#template",thisTemplate)>
			</cfloop>
		</cfif>
		<cfparam name="controllerTemplate" default="">
		<cfparam name="modelTemplate" default="">
		<cfparam name="viewTemplate" default="">
		<cfparam name="otherTemplate" default="">
		...
		<cfif isDefined("arguments.lIgnoreOnInsert")>
			<cfset variables.lIgnoreOnInsert = arguments.lIgnoreOnInsert>
		<cfelse>
			<cfset variables.lIgnoreOnInsert = "">
		</cfif>
		
		<cfif isDefined("arguments.lIgnoreOnUpdate")>
			<cfset variables.lIgnoreOnUpdate = arguments.lIgnoreOnUpdate>
		<cfelse>
			<cfset variables.lIgnoreOnUpdate = "">
		</cfif>
		
		<cfif isDefined("arguments.activeIndicator")>
			<cfset variables.activeIndicator = arguments.activeIndicator>
		<cfelse>
			<cfset variables.activeIndicator = "">
		</cfif>
		
		<cfif isDefined("arguments.deletedIndicator")>
			<cfset variables.deletedIndicator = arguments.deletedIndicator>
		<cfelse>
			<cfset variables.deletedIndicator = "">
		</cfif>
	...
	<cffunction name="introspectDB" returntype="metadata" output="Yes" 
		...
		<cfelse>
			<cfset lValidTables = listSort(ValueList(qTables.tablename),"textnocase","Asc")>
		</cfif>
		...
		<cfset save()>
		<cfoutput>Introspection complete.<br /></cfoutput>
		<cfflush>
		<cfreturn this>
	</cffunction>
	...
	<cffunction name="build" returntype="void" output="Yes" 
		...
		<cfargument name="cftemplate" type="any" required="Yes" 
					hint="I am the cftemplate.cfc that will be used to generate the code."/>
		...
		<cfargument name="baseURL" type="string" required="No" default=""
					hint="I am the URL path to the application" />
		...
		<cfset setDestinationFilePath(arguments.destinationFilePath)>
		...
		<cfset cftemplate.progressReport(message="Beginning #ListFirst(arguments.template,"/")# Code Generation.")>
		...
		<cfloop list="#arguments.lTables#" index="thisTable">
			...
			<cfloop index="i" from="1" to="#ArrayLen(aTemplateFiles)#">
				<cfif aTemplateFiles[i].perObject>
					...
					<!--- Generate the code --->
					<cfif aTemplateFiles[i].suffix IS "cfc" OR aTemplateFiles[i].suffix IS "as" <!--- OR aTemplateFiles[i].MVCpath CONTAINS "#variables.OSdelimiter#model" ---> >
	...
	<cffunction name="getTemplates" returntype="query" output="No" hint="I get a recordset containing the available templates.">
		<cfargument name="templateType" required="No">
		<cfset var qTemplates = 0>
		<!--- There can be model, view, controller and custom subdirectories --->
		<!--- Get a recordset of the subdirectories below the templates directory --->
		<cfif isDefined("arguments.templateType")>
			<cfdirectory action="LIST" directory="#GetDirectoryFromPath(getCurrentTemplatePath())#../templates/#arguments.templateType#/" name="qTemplates">
		<cfelse>
...
<scaffolding>
	...
	<cffunction name="getTemplate" returntype="string" output="No" hint="I return the template list.">
		<cfreturn variables.template>
	</cffunction>
	
	<cffunction name="getControllerTemplate" returntype="string" output="No" hint="I return the controller template.">
		<cfreturn variables.controllertemplate>
	</cffunction>
	
	<cffunction name="getModelTemplate" returntype="string" output="No" hint="I return the model template.">
		<cfreturn variables.modeltemplate>
	</cffunction>
	
	<cffunction name="getViewTemplate" returntype="string" output="No" hint="I return the view template.">
		<cfreturn variables.viewtemplate>
	</cffunction>
	...
	<cffunction name="getLIgnoreOnInsert" returntype="string" output="No" hint="I return the list of fields to be ignored when inserting records.">
		<cfreturn variables.lIgnoreOnInsert>
	</cffunction>
	
	<cffunction name="getLIgnoreOnUpdate" returntype="string" output="No" hint="I return the list of fields to be ignored when updating records.">
		<cfreturn variables.lIgnoreOnUpdate>
	</cffunction>
	
	<cffunction name="getActiveIndicator" returntype="string" output="No" hint="I return the list of fields to be ignored when inserting records">
		<cfreturn variables.activeIndicator>
	</cffunction>
	
	<cffunction name="getDeletedIndicator" returntype="string" output="No" hint="I return the list of fields to be ignored when inserting records">
		<cfreturn variables.deletedIndicator>
	</cffunction>

	<cffunction name="getLSkipColumns" returntype="string" output="No" hint="I return the list of fields to be skipped on forms.">
		<cfreturn ListAppend(ListAppend(ListAppend(getLIgnoreOnInsert(),getLIgnoreOnUpdate()),getActiveIndicator()),getDeletedIndicator())>
	</cffunction>
...
<!--- *** 													  **** --->
	...
	<cffunction name="read" returntype="any" output="Yes" hint="I read the XML Metadata file and populate the local copy.">
	...
	<cffunction name="setConfigInXML" returntype="boolean" output="No" hint="I set up the basic configuration information in the XML.">
		<cfset var fieldlist="project,datasource,username,password,template,author,authorEmail,copyright,licence,version,lignoreoninsert,lignoreonupdate,activeindicator,deletedindicator">
	...
	<cffunction name="getConfigFromXML" returntype="boolean" output="Yes" hint="I get the basic configuration information from the XML.">
		<cfset var fieldlist="project,datasource,username,password,template,author,authorEmail,copyright,licence,version,controllerTemplate,modelTemplate,viewTemplate,otherTemplate">
	...
	<cffunction name="createObjectTag" returntype="numeric" output="No" 
		...
		<!--- If the object wasn't found then add it and set the name, alias, label and package --->	
		<cfif thisIndex IS 0>
			...
			<cfset variables.xScaffoldingConfig.scaffolding.objects.object[thisIndex].XmlAttributes["package"] = arguments.alias>
		</cfif>
	...
	<cffunction name="updateBaseFieldPropertiesFromQuery" returntype="void" output="No" 
		...
		<cfloop query="arguments.qTableData">
			...
			<cfset thisFieldIndex = createFieldTag(objectIndex=objectIndex,name=lFirst(arguments.qTableData.column_name))>
	...
	<cffunction name="updateAdditionalFieldPropertiesFromQuery" returntype="void" output="No" 
		...
		<cfloop query="arguments.qTableData">
			...
			<!--- The values of these XML attribute values never get overwritten, if they are already present. --->
			...
			<cfset setAttributeValue(stProperties,"showOnDisplay","true")>
			<cfif listFindNoCase(getLSkipColumns(),arguments.qTableData.column_name)>
				<cfset setAttributeValue(stProperties,"showOnForm","false")>
			<cfelse>
			...
			<cfif stProperties.type IS "date" AND arguments.qTableData.column_name CONTAINS "time">
				...
				<cfset setAttributeValue(stProperties,"validation","time")>
			<cfelseif stProperties.type IS "date">
				...
				<cfset setAttributeValue(stProperties,"validation","date")>
			<cfelseif isDefined("stProperties.parent") AND stProperties.parent IS NOT "">
				...
				<cfset setAttributeValue(stProperties,"validation","parent")>
			<cfelseif stProperties.type IS "boolean">
				...
				<cfset setAttributeValue(stProperties,"validation","boolean")>
			...
			<cfelseif arguments.qTableData.type_name IS "money">
				...
				<cfset setAttributeValue(stProperties,"validation","currency")>
			<cfelseif stProperties.fuseDocType IS "integer">
				<cfset setAttributeValue(stProperties,"formType","Text")>
				<cfset setAttributeValue(stProperties,"format","Integer")>
				<cfset setAttributeValue(stProperties,"size","15")>
				<cfset setAttributeValue(stProperties,"maxlength",15)>
				<cfset setAttributeValue(stProperties,"validation","integer")>
			<cfelseif stProperties.type IS "numeric">
				<cfset setAttributeValue(stProperties,"formType","Text")>
				<cfset setAttributeValue(stProperties,"format","Number(9.99)")>
				<cfset setAttributeValue(stProperties,"size","15")>
				<cfset setAttributeValue(stProperties,"maxlength",15)>
				<cfset setAttributeValue(stProperties,"validation","real")>
			<cfelseif (arguments.qTableData.column_name CONTAINS "zip_code"
					OR arguments.qTableData.column_name CONTAINS "zipcode"
					OR arguments.qTableData.column_name CONTAINS "post_code"
					OR arguments.qTableData.column_name CONTAINS "postal_code"
					OR arguments.qTableData.column_name CONTAINS "postcode"
					OR arguments.qTableData.column_name CONTAINS "postalcode")
					AND arguments.qTableData.length GE 5>
				<cfset setAttributeValue(stProperties,"formType","Text")>
				<cfset setAttributeValue(stProperties,"format","Trim")>
				<cfset setAttributeValue(stProperties,"size","15")>
				<cfset setAttributeValue(stProperties,"maxlength",min(arguments.qTableData.length,getBuffer()))>
				<cfset setAttributeValue(stProperties,"validation","zip_code")>
			<cfelseif arguments.qTableData.column_name CONTAINS "email" AND arguments.qTableData.length GT 6>
				<cfset setAttributeValue(stProperties,"formType","Text")>
				<cfset setAttributeValue(stProperties,"format","Trim")>
				<cfset setAttributeValue(stProperties,"size","30")>
				<cfset setAttributeValue(stProperties,"maxlength",min(arguments.qTableData.length,getBuffer()))>
				<cfset setAttributeValue(stProperties,"validation","email")>
			<cfelseif arguments.qTableData.column_name CONTAINS "phone" AND arguments.qTableData.length GT 6>
				<cfset setAttributeValue(stProperties,"formType","Text")>
				<cfset setAttributeValue(stProperties,"format","Trim")>
				<cfset setAttributeValue(stProperties,"size","15")>
				<cfset setAttributeValue(stProperties,"maxlength",min(arguments.qTableData.length,getBuffer()))>
				<cfset setAttributeValue(stProperties,"validation","phone")>
			<cfelseif arguments.qTableData.column_name CONTAINS "url" AND arguments.qTableData.length GT 15>
				<cfset setAttributeValue(stProperties,"formType","Text")>
				<cfset setAttributeValue(stProperties,"format","Trim")>
				<cfset setAttributeValue(stProperties,"size","30")>
				<cfset setAttributeValue(stProperties,"maxlength",min(arguments.qTableData.length,getBuffer()))>
				<cfset setAttributeValue(stProperties,"validation","url")>
			<cfelseif arguments.qTableData.length gt 100 >
				<cfset setAttributeValue(stProperties,"formType","Textarea")>
				<cfset setAttributeValue(stProperties,"format","Trim")>
				<cfset setAttributeValue(stProperties,"size","30x4")>
				<cfset setAttributeValue(stProperties,"maxlength",min(arguments.qTableData.length,getBuffer()))>
				<cfset setAttributeValue(stProperties,"validation","none")>
			<cfelse>
				<cfset setAttributeValue(stProperties,"formType","Text")>
				<cfset setAttributeValue(stProperties,"format","Trim")>
				<cfset setAttributeValue(stProperties,"size","30")>
	...
	<cffunction name="getSelectedTableLabel" returntype="string" output="No" >
		...
		<cfelseif structKeyExists(xTable[1].xmlAttributes,"alias")>
			<cfreturn cleanLabelText(xTable[1].xmlAttributes["alias"])>
		<cfelse>
	...
	<cffunction name="setBaseURL" returntype="void" output="No">
		<cfargument name="baseURL" type="string" required="Yes" />
		<cfset variables.baseURL = trim(arguments.baseURL)>
	</cffunction>
	<cffunction name="getBaseURL" returntype="string" output="No">
		<cfreturn variables.baseURL>
	</cffunction>
	<cffunction name="getBaseCFCPath" returntype="string" output="No">
		<cfreturn listChangeDelims(variables.baseURL,".","/")>
	</cffunction>
	...
	<cffunction name="getLTables" returntype="string" output="No">
		<cfreturn variables.lTables>
	</cffunction>
	
	<!--- Get and Set destinationFilePath --->
	<cffunction name="setDestinationFilePath" returntype="void" output="No" >
		<cfargument name="destinationFilePath" type="string" required="Yes" />
		<cfset variables.destinationFilePath = arguments.destinationFilePath>
	</cffunction>
	<cffunction name="getDestinationFilePath" returntype="string" output="No">
		<cfreturn variables.destinationFilePath>
	</cffunction>
...
<!--- 																									 --->
	...
	<cffunction name="getFieldsFromXML" returntype="array" output="No" 
		...
		<cfargument name="select" type="string" required="No" default="All" hint="I am the selction criteria for the fields to be output. Can be All(default), showOnForm, showOnList, showOnDisplay, insert, update.">
		...
		<cfloop index="j" from="1" to="#arrayLen(xFields)#">
			<cfif NOT structKeyExists(xFields[j].XmlAttributes,"alias")>
				<cfset thisAlias = lFirst(xFields[j].XmlAttributes["name"])>
			<cfelse>
				<cfset thisAlias = lFirst(xFields[j].XmlAttributes["alias"])>
			</cfif>
			
			<cfif arguments.select IS "All"
				OR (arguments.select IS "showOnForm" AND ((NOT structKeyExists(xFields[j].XmlAttributes,"showOnForm")) OR xFields[j].XmlAttributes.showOnForm))
				OR (arguments.select IS "showOnList" AND ((NOT structKeyExists(xFields[j].XmlAttributes,"showOnList")) OR xFields[j].XmlAttributes.showOnForm))
				OR (arguments.select IS "showOnDisplay" AND ((NOT structKeyExists(xFields[j].XmlAttributes,"showOnDisplay")) OR xFields[j].XmlAttributes.showOnForm))
				OR (arguments.select IS "update" AND (NOT ListFindNoCase(getLIgnoreOnUpdate(),thisAlias)))
				OR (arguments.select IS "insert" AND (NOT ListFindNoCase(getLIgnoreOnInsert(),thisAlias)))
				 >
				<cfset i = i + 1>
				<cfset arrayAppend(aFields,xFields[j].XmlAttributes)>
				<cfset aFields[i]["table"] = uFirst(arguments.table)>
				<!--- Set default values on missing entries in the XML --->
				<cfif NOT structKeyExists(aFields[i],"alias")>
					<cfset aFields[i]["alias"] = lFirst(aFields[i]["name"])>
				</cfif>
				...
				<cfif NOT structKeyExists(aFields[i],"showOnDisplay")>
					<cfset aFields[i]["showOnDisplay"] = "true">
				</cfif>
				<cfif NOT structKeyExists(aFields[i],"type")>
					<cfset aFields[i]["type"] = "string">
				</cfif>
				<cfif NOT structKeyExists(aFields[i],"fuseDocType")>
					<cfset aFields[i]["fuseDocType"] = "string">
				</cfif>
				<cfif NOT structKeyExists(aFields[i],"formType")>
					...
					<cfelseif aFields[i].type IS "date">
						<cfif aFields[i].name CONTAINS "time">
							<cfset aFields[i]["formType"] = "Time">
						<cfelse>
				...
				<cfif NOT structKeyExists(aFields[i],"validation")>
					<cfif structKeyExists(aFields[i],"parent")>
						<cfset aFields[i]["validation"] = "parent">
					<cfelseif aFields[i].formType IS "Time">
						<cfset aFields[i]["validation"] = "time">
					<cfelseif aFields[i].formType IS "Date" OR aFields[i].formType IS "Calendar">
						<cfset aFields[i]["validation"] = "date">
					<cfelseif aFields[i].type IS "boolean">
						<cfset aFields[i]["validation"] = "boolean">
					<cfelseif aFields[i].type IS "money">
						<cfset aFields[i]["validation"] = "currency">
					<cfelseif aFields[i].fuseDocType IS "integer">
						<cfset aFields[i]["validation"] = "integer">
					<cfelseif aFields[i].type IS "numeric">
						<cfset aFields[i]["validation"] = "real">
					<cfelseif aFields[i].type IS "string" AND aFields[i].name CONTAINS "zip_code" AND structKeyExists(aFields[i],"maxlength") AND aFields[i].maxlength GE 5>
						<cfset aFields[i]["validation"] = "zip_code">
					<cfelseif aFields[i].type IS "string" AND aFields[i].name CONTAINS "zipcode" AND structKeyExists(aFields[i],"maxlength") AND aFields[i].maxlength GE 5>
						<cfset aFields[i]["validation"] = "zip_code">
					<cfelseif aFields[i].type IS "string" AND aFields[i].name CONTAINS "postcode" AND structKeyExists(aFields[i],"maxlength") AND aFields[i].maxlength GE 5>
						<cfset aFields[i]["validation"] = "zip_code">
					<cfelseif aFields[i].type IS "string" AND aFields[i].name CONTAINS "post_code" AND structKeyExists(aFields[i],"maxlength") AND aFields[i].maxlength GE 5>
						<cfset aFields[i]["validation"] = "zip_code">
					<cfelseif aFields[i].type IS "string" AND aFields[i].name CONTAINS "email" AND structKeyExists(aFields[i],"maxlength") AND aFields[i].maxlength GT 6>
						<cfset aFields[i]["validation"] = "email">
					<cfelseif aFields[i].type IS "string" AND aFields[i].name CONTAINS "phone" AND structKeyExists(aFields[i],"maxlength") AND aFields[i].maxlength GT 6>
						<cfset aFields[i]["validation"] = "phone">
					<cfelseif aFields[i].type IS "string" AND aFields[i].name CONTAINS "url" AND structKeyExists(aFields[i],"maxlength") AND aFields[i].maxlength GT 6>
						<cfset aFields[i]["validation"] = "url">
					<cfelse>
						<cfset aFields[i]["validation"] = "none">
					</cfif>
				</cfif>
	...
	<cffunction name="getFieldFromXML" returntype="struct" output="No" 
				hint="I return a structure of a particular field for a table." >
		<cfargument name="table" type="string" required="Yes" hint="I am the name of the table who's fields are to be output.">
		<cfargument name="field" type="string" required="Yes" hint="I am the name of the field required.">
		<cfargument name="select" type="string" required="No" default="name" hint="Choose either the name or alias.">
		<cfset var i = 0>
		<cfset var aFields = getFieldsFromXML(arguments.table)>
		<cfset var stField = structNew()>
		
		<cfloop from="1" to="#arrayLen(aFields)#" index="i">
			<cfif structKeyExists(aFields[i],arguments.select) AND arguments.field IS aFields[i][arguments.select]>
				<cfreturn aFields[i]>
			<cfelseif arguments.field IS aFields[i].name>
				<cfreturn aFields[i]>
			</cfif>
		</cfloop>
		<cfreturn structNew()>
	...
	<cffunction name="getFieldListFromXML" returntype="string" output="No" 
		...
		<cfset var aFields = getFieldsFromXML(arguments.table)>
		<cfset var lFields = "">
		<cfset var lSkipFields = "">
		<cfif arguments.select IS "insert">
			<cfset lSkipFields = getLIgnoreOnInsert()>
			<cfset arguments.select = "All">
		<cfelseif arguments.select IS "update">
			<cfset lSkipFields = getLIgnoreOnUpdate()>
			<cfset arguments.select = "All">
		</cfif>
		
		<cfloop from="1" to="#arrayLen(aFields)#" index="i">
			<cfif arguments.select IS "All" OR (structKeyExists(aFields[i],arguments.select) AND aFields[i][arguments.select])>
				<cfif structKeyExists(aFields[i],"alias") AND NOT ListFindNoCase(lSkipFields,aFields[i].alias)>
					<cfset lFields = ListAppend(lFields,aFields[i].alias)>
				<cfelseif NOT ListFindNoCase(lSkipFields,aFields[i].name)>
	...
	<cffunction name="getJoinedFieldListFromXML" returntype="string" output="No" 
		...
		<cfargument name="select" type="string" required="No" default="All" hint="The type of field, can be showOnList, showOnForm, showOnDisplay.">
		<cfset var i = 0>
		<cfset var aJoinedFields = getJoinedFieldsFromXML(arguments.table)>
		<cfset var lFields = "">
		
		<cfloop from="1" to="#arrayLen(aJoinedFields)#" index="i">
			<cfif arguments.select IS "All" OR (structKeyExists(aJoinedFields[i],arguments.select) AND aJoinedFields[i][arguments.select])>
				<cfif structKeyExists(aJoinedFields[i],"alias")>
					<cfset lFields = ListAppend(lFields,lFirst(aJoinedFields[i].prefix) & uFirst(aJoinedFields[i].alias))>
				<cfelse>
					<cfset lFields = ListAppend(lFields,lFirst(aJoinedFields[i].prefix) & uFirst(aJoinedFields[i].name))>
				</cfif>
			</cfif>
		</cfloop>
	...
	<cffunction name="getJoinedFieldsFromXML" returntype="array" output="No" 
		...
		<cfargument name="table" type="string" required="Yes" hint="I am the name of the table who's joined tables fields are to be output.">
		<cfset var i = 0>
		<cfset var aJoinedObjects = getRelationshipsFromXML(arguments.table,"manyToOne")>
		<cfset var aJoinedFields = arrayNew(1)>
		<cfset var aAllJoinedFields = arrayNew(1)>
		
		<cfloop from="1" to="#arrayLen(aJoinedObjects)#" index="i">
			<cfset aJoinedFields = getFieldsFromXML(aJoinedObjects[i].name)>
			<cfloop from="1" to="#arrayLen(aJoinedFields)#" index="j">
				<cfset aJoinedFields[j]["prefix"] = aJoinedObjects[i].JoinedFieldPrefix>
			</cfloop>
			
			<cfset aAllJoinedFields = ArrayConcat(aAllJoinedFields,aJoinedFields)>
		</cfloop>
		
		<cfreturn aAllJoinedFields>
	</cffunction>
	...
	<cffunction name="getRelationshipsFromXML" returntype="array" output="No" 
		...
		<cfargument name="removeDuplicateTables" required="No" default="No" hint="For the construction of validators we only need each table once.">
		...
		<cfset var lJoinedFields = "">
		<cfset var lJoinedTables = "">
		...
		<cfloop index="i" from="1" to="#arrayLen(xRelationships)#">
			...
			<cfif removeDuplicateTables>
				<cfif NOT ListFindNoCase(lJoinedTables,xRelationships[i].XmlAttributes.name)>
					<cfset lJoinedTables = listAppend(lJoinedTables,xRelationships[i].XmlAttributes.name)>
					<cfset arrayAppend(aRelationships,stData)>
				</cfif>
			<cfelse>
				<cfset temp = xRelationships[i].XmlChildren[1].XmlAttributes.from>
				<cfset stData["Required"] = getFieldFromXML(arguments.table,temp).required>
				<cfset stData["JoinType"] = iif(structkeyExists(xRelationships[i].XmlAttributes,"fkName") AND stData.required,"'INNER'","'LEFT OUTER'")>
				
				<cfset stData["Links"] = arrayNew(1)>
				<cfset stData["lPKFields"] = "">
				<cfloop index="j" from="1" to="#arrayLen(xRelationships[i].XmlChildren)#">
					...
					<cfset stData.lPKFields = listAppend(stData.lPKFields,xRelationships[i].XmlChildren[j].XmlAttributes.to)>
				</cfloop>
				
				<!--- Make a field name prefix from the field name used on the link --->
				<cfif structKeyExists(stData.Links[1],"from") AND right(stData.Links[1]["from"],2) IS "Id" >
					<cfset stData["joinedFieldPrefix"] = left(stData.Links[1]["from"],Len(stData.Links[1]["from"])-2) >
				<cfelseif structKeyExists(stData.Links[1],"from")>
					<cfset stData["joinedFieldPrefix"] = stData.Links[1]["from"] >
				<cfelse>
					<cfset stData["joinedFieldPrefix"] = stData["Alias"]>
				</cfif>
				<!--- Look to see if there is a suitable Foreign Key to use to use as a field name prefix --->
				<cfif structKeyExists(xRelationships[i].XmlAttributes,"fkName")>
					<cfset stData["ForeignKeyName"] = xRelationships[i].XmlAttributes.fkName>
					<cfif variables.DbType IS "mssql" AND listLen(stData["ForeignKeyName"],"_") IS 3>
						<cfset stData["joinedFieldPrefix"] = listLast(stData["ForeignKeyName"],"_")>
					</cfif>
				</cfif>
				
				<!--- Look to see if there is a suitable name, title, label or description field in the related table that we can use in combo boxes etc. --->
				<cfset lJoinedFields = getFieldListFromXML(stData.name)>
				<cfif listFindNoCase(lJoinedFields, "name")>
					<cfset stData["titleField"] = "name">
				<cfelseif listFindNoCase(lJoinedFields, "title")>
					<cfset stData["titleField"] = "title">
				<cfelseif listFindNoCase(lJoinedFields, "label")>
					<cfset stData["titleField"] = "label">
				<cfelseif listFindNoCase(lJoinedFields, "description")>
					<cfset stData["titleField"] = "description">
				<cfelse>
					<cfset stData["titleField"] = "">
				</cfif>
	...
	<cffunction name="getFKFieldList" returntype="string" output="No" 
				hint="I return a list containing the fields that are foreign keys to tables related to the selected table from the XML Configuration.">
		<cfargument name="table" required="Yes" hint="I am the name of the table who's realtionships are to be output.">
		<cfargument name="type" required="Yes" hint="I am the type of relationship required">
		<cfset var i = 0>
		<cfset var j = 0>
		<cfset var lFKFieldList = "">
		<cfset var quotedTable = "'#arguments.table#'">
		<cfset var xRelationships = XmlSearch(variables.xScaffoldingConfig,"/scaffolding/objects/object[@name=#quotedTable#]/#arguments.type#")>
		
		<cfloop index="i" from="1" to="#arrayLen(xRelationships)#">
			<cfloop index="j" from="1" to="#arrayLen(xRelationships[i].XmlChildren)#">
				<cfset lFKFieldList = ListAppend(lFKFieldList,xRelationships[i].XmlChildren[j].XmlAttributes.from)>
			</cfloop>
		</cfloop>
		
		<cfreturn lFKFieldList>
	</cffunction>
	
	<cffunction name="getDottedPath" returntype="string" output="no" 
				hint="I return the Path to the model, which consists of the app, the project and the object.">
		...
		<cfset var pathInfo = replace(getDirectoryFromPath(variables.baseURL),"/",".","all") >
		<cfset var dottedPath = "" >
		
		<cfif right(destPath,11) IS ".generated.">
			<cfset destPath = left(destPath,len(destPath)-10)>
			<cfset dottedPath = RemoveChars(destPath,1,FindNoCase(pathInfo,destPath,1)) & arguments.objectName >
		<cfelseif arguments.datasource IS "">
			<cfset dottedPath = RemoveChars(destPath,1,FindNoCase(pathInfo,destPath,1)) & arguments.objectName >
		<cfelseif right(destPath,8) IS ".config.">
			<cfset destPath = left(destPath,len(destPath)-7)>
			<cfset dottedPath = RemoveChars(destPath,1,FindNoCase(pathInfo,destPath,1)) & "m#arguments.datasource#." & arguments.objectName >
		<cfelse>
