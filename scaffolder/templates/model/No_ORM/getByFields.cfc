...
<<cfoutput>>
	<cffunction name="getByFields" access="public" output="false" returntype="query">
		<<cfloop from="1" to="$$ArrayLen(aFields)$$" index="i">><cfargument name="$$aFields[i].alias$$" type="<<cfif aFields[i].type IS "boolean">>Any<<cfelse>>$$aFields[i].type$$<</cfif>>" required="false" />
		<</cfloop>>
		<!--- Joined Fields ---><!--- 
		<<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">><cfargument name="$$lFirst(aJoinedFields[i].prefix)$$$$uFirst(aJoinedFields[i].alias)$$" type="<<cfif aJoinedFields[i].type IS "boolean">>Any<<cfelse>>$$aJoinedFields[i].type$$<</cfif>>" required="false" />
		<</cfloop>> --->
		...
		<cfquery name="qList" datasource="#variables.dsn#">
			SELECT 	<cfif structKeyExists(arguments,"maxrows") AND arguments.maxrows GT 0>TOP #arguments.maxrows#</cfif>
					<<cfloop from="1" to="$$ArrayLen(aFields)$$" index="i">>[$$tableName$$].[$$aFields[i].name$$] AS [$$aFields[i].alias$$]<<cfif i NEQ ArrayLen(aFields)>>,$$chr(10)$$<</cfif>>					<</cfloop>>
					<!--- <<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>[$$aJoinedFields[i].table$$].[$$aJoinedFields[i].name$$] AS [$$lFirst(aJoinedFields[i].prefix)$$$$uFirst(aJoinedFields[i].alias)$$]<<cfif i NEQ ArrayLen(aJoinedFields)>>,$$chr(10)$$<</cfif>>					<</cfloop>> --->
		...
		<!--- <<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>
			<cfif structKeyExists(arguments,"$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$") and len(arguments.$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$)>
				AND	[$$aJoinedFields[i].prefix$$].[$$aJoinedFields[i].name$$] = <cfqueryparam value="#arguments.$$lFirst(aJoinedFields[i].prefix)$$$$uFirst(aJoinedFields[i].alias)$$#" CFSQLType="$$aJoinedFields[i].SQLType$$" />
			</cfif><</cfloop>> --->
			<cfif startrow IS NOT 1>
				AND $$lPKFields$$ NOT IN(SELECT TOP #val(startrow - 1)# $$lPKFields$$ FROM $$tableName$$ 
				...
				<!--- <<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>
					<cfif structKeyExists(arguments,"$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$") and len(arguments.$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$)>
						AND	[$$aJoinedFields[i].table$$].[$$aJoinedFields[i].name$$] = <cfqueryparam value="#arguments.$$aJoinedFields[i].prefix$$$$aJoinedFields[i].alias$$#" CFSQLType="$$aJoinedFields[i].SQLType$$" />
					</cfif><</cfloop>> --->
					<cfif structKeyExists(arguments, "sortByFieldList") and len(arguments.sortByFieldList)>
					ORDER BY #orderBy#
					</cfif>
