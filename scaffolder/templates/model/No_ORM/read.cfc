	...
	<cffunction name="read" access="public" output="false" returntype="boolean">
		...
		<cftry>
			<cfquery name="qRead" datasource="#variables.dsn#">
				SELECT	<<cfloop from="1" to="$$ArrayLen(aFields)$$" index="i">>[$$tableName$$].[$$aFields[i].name$$] AS [$$aFields[i].alias$$]<<cfif i NEQ ArrayLen(aFields)>>,$$chr(10)$$<</cfif>>						<</cfloop>>
				...
				FROM	[$$tableName$$]
				<!--- <<cfloop from="1" to="$$ArrayLen(aJoinedObjects)$$" index="i">>$$aJoinedObjects[i].joinType$$ JOIN [$$aJoinedObjects[i].name$$] ON 
			...
			<cfcatch type="database">
				...
				<cfset qRead = queryNew("<<cfloop from="1" to="$$ArrayLen(aFields)$$" index="i">>$$aFields[i].alias$$<<cfif i LT ArrayLen(aFields)>>,<</cfif>><</cfloop>><<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>,$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$<</cfloop>>") />
			</cfcatch>
		</cftry>
		...
		<cfreturn (qRead.recordCount GT 0)>
	</cffunction>
<</cfoutput>>
