...
<<cfoutput>>
	<cffunction name="get$$objectName$$" access="public" output="false" returntype="$$objectName$$Bean">
		<<cfloop from="1" to="$$ArrayLen(aPKFields)$$" index="i">>
		<cfargument name="$$aPKFields[i].alias$$" type="$$aPKFields[i].type$$" required="true" /><</cfloop>>
		<<cfif ListFindNoCase(lFields,"$$oMetadata.getActiveIndicator()$$")>><cfargument name="$$oMetadata.getActiveIndicator()$$" type="Any" required="false" default="true" /><</cfif>>
		<<cfif ListFindNoCase(lFields,"$$oMetadata.getDeletedIndicator()$$")>><cfargument name="$$oMetadata.getDeletedIndicator()$$" type="Any" required="false" default="false" /><</cfif>>
