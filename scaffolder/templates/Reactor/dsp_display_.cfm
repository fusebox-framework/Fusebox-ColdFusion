...
<<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>
<cfset stJoinedFields["$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$"] = "$$aJoinedFields[i].table$$">><</cfloop>>
...
<cfif isDefined("o$$objectname$$")>
	...
	<cfloop list="#fieldlist#" index="thisField">
		<cfif ListFindNoCase(_joinedFieldlist,thisField)>
			<cfset thisFieldBaseName = RemoveChars(thisField,1,Len(stJoinedFields[thisField]))>
			<cfset setvariable("variables.#thisField#", evaluate("o$$objectname$$.get" & stJoinedFields[thisField] & "().get" & thisFieldBaseName & "()"))>
		<cfelse>
	...
	<table width="590" border="0" cellpadding="2" cellspacing="2" summary="This table shows details of a single $$objectname$$ record." class="">
		...
		<cfloop list="#fieldlist#" index="thisField">
		  <cfswitch expression="#thisField#">
			...
			<<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">><<cfif NOT ListFindNoCase(lFields,aJoinedFields[i].alias)>>
			<cfcase value="$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$">
				<tr>
				  ...
				  <cfelse>
					...
					<td>
						<cftry>#$$Format("variables.$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$","$$aJoinedFields[i].format$$")$$#<cfcatch>#variables.$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$#</cfcatch></cftry>
					</td>
				</tr>
			</cfcase><</cfif>><</cfloop>>
		  </cfswitch>
