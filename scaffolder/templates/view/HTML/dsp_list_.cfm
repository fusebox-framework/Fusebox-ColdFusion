...
<fusedoc fuse="$RCSfile: dsp_list_$$objectName$$.cfm,v $" language="ColdFusion 7.01" version="2.0"  >
	<responsibilities>
		...
		Each record in the list has a link for viewing, deleting, and editing the selected 
			$$objectName$$ record.
		If there are more records than can be displayed I show a list of the pages with link to
			each page and prev and next buttons.
		I also display a link to add a new $$objectName$$.
		You can customise the columns to be displayed and the order of display by setting variables.fieldlist.
	</responsibilities>
	<properties>
	...
	<io>
		<in>
			...
			<string name="XFA.View" scope="request" comments="link to view a record" />
			<string name="XFA.Edit" scope="request" comments="link to edit a record" />
			<string name="XFA.Add" scope="request" comments="link to add a record" />
			<string name="XFA.Delete" scope="request" comments="link to delete a record" />
			<string name="XFA.Prev" scope="request" comments="link to next page" />
			<string name="XFA.Next" scope="request" comments="link to next page" />
			...
			<list name="fieldlist" scope="variables" optional="Yes" 
				default="$$lAllFields$$" 
				comments="List of fields to display and the order to display in." />
		</in>
		<out>
...
--->
<cfparam name="XFA.View">
<cfparam name="XFA.Edit">
				...
				<cfcase value="$$lFirst(aFields[i].alias)$$">
					<th width="" align="right" class="standard">
						<cfif isDefined("attributes._listSortByFieldList") AND attributes._listSortByFieldList IS "$$aFields[i].table$$|$$lFirst(aFields[i].alias)$$|ASC">
							<a href="#myFusebox.getSelf()#?fuseaction=#XFA.Sort#&_listsortByFieldList=$$objectName$$|$$lFirst(aFields[i].alias)$$|DESC">$$aFields[i].label$$</a>
						<cfelse>
							<a href="#myFusebox.getSelf()#?fuseaction=#XFA.Sort#&_listsortByFieldList=$$objectName$$|$$lFirst(aFields[i].alias)$$|ASC">$$aFields[i].label$$</a>
						</cfif>
					</th>
				</cfcase>
				<</cfloop>>
			...
			<tr>
				<cfloop list="#variables.fieldlist#" index="thisField">
					<cfswitch expression="#thisField#">
						<<cfloop from="1" to="$$ArrayLen(aFields)$$" index="i">>
						<cfcase value="$$lFirst(aFields[i].alias)$$">
							<<cfif aFields[i].formType IS "Password">><td align="right" class="standard">&nbsp;</td><<cfelse>>
							<td align="right" class="standard"><cftry>#$$Format("q$$uFirst(objectName)$$.$$lFirst(aFields[i].alias)$$","$$aFields[i].format$$")$$#<cfcatch>#q$$uFirst(objectName)$$.$$lFirst(aFields[i].alias)$$#</cfcatch></cftry>&nbsp;</td><</cfif>>
						</cfcase>
						<</cfloop>>
						<!--- Joined Fields --->
						<!--- <<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>
						<cfcase value="$$lFirst(aJoinedFields[i].table)$$$$uFirst(aJoinedFields[i].alias)$$">
							...
							<td align="right" class="standard"><cftry>#$$Format("q$$uFirst(objectName)$$.$$lFirst(aJoinedFields[i].prefix)$$$$uFirst(aJoinedFields[i].alias)$$","$$aJoinedFields[i].format$$")$$#<cfcatch>#q$$objectName$$.$$lFirst(aJoinedFields[i].table)$$$$uFirst(aJoinedFields[i].alias)$$#</cfcatch></cftry>&nbsp;</td><</cfif>>
						</cfcase>
						<</cfloop>> --->
					</cfswitch>
				</cfloop>
				<td align="center" class="standard">
					<cfif isDefined("XFA.view")>[<a href="#myFusebox.getSelf()##appendParam(appendParam(pageParams,"fuseaction",XFA.view),"$$lPKFields$$",$$lPKFields$$)#">View</a>]</cfif>
					<cfif isDefined("XFA.edit") >[<a href="#myFusebox.getSelf()##appendParam(appendParam(pageParams,"fuseaction",XFA.edit),"$$lPKFields$$",$$lPKFields$$)#">Edit</a>]</cfif>
...
<span class="standard">
	...
	<<cfif ListFindNoCase(lFields,"$$oMetadata.getActiveIndicator()$$")>>
	<cfoutput>
		<cfif isDefined("attributes.$$oMetadata.getActiveIndicator()$$") AND  attributes.$$oMetadata.getActiveIndicator()$$ EQ False >
			[<a href="#myFusebox.getSelf()##appendParam(pageParams,"fuseaction",XFA.page)#&amp;$$oMetadata.getActiveIndicator()$$=true">Hide Inactive $$objectName$$ Records</a>]
		<cfelse>
			[<a href="#myFusebox.getSelf()##appendParam(pageParams,"fuseaction",XFA.page)#&amp;$$oMetadata.getActiveIndicator()$$=false">Show Inactive $$objectName$$ Records</a>]
		</cfif>
	</cfoutput>
	<</cfif>>
	<<cfif ListFindNoCase(lFields,"$$oMetadata.getDeletedIndicator()$$")>>
	<cfoutput>
		<cfif isDefined("attributes.$$oMetadata.getDeletedIndicator()$$") AND  attributes.$$oMetadata.getDeletedIndicator()$$ EQ True >
			[<a href="#myFusebox.getSelf()##appendParam(pageParams,"fuseaction",XFA.page)#&amp;$$oMetadata.getDeletedIndicator()$$=false">Show Deleted $$objectName$$ Records</a>]
		<cfelse>
			[<a href="#myFusebox.getSelf()##appendParam(pageParams,"fuseaction",XFA.page)#&amp;$$oMetadata.getDeletedIndicator()$$=true">Show Deleted $$objectName$$ Records</a>]
		</cfif>
	</cfoutput>
	<</cfif>>
