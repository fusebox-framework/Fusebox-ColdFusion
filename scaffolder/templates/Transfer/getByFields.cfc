...
<<cfoutput>>
	<cffunction name="getByFields" access="public" output="false" returntype="query">
		...
		<cfargument name="$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$" type="<<cfif aJoinedFields[i].type IS "boolean">>Any<<cfelse>>$$aJoinedFields[i].type$$<</cfif>>" required="false" /><</cfloop>>
		...
		<cfquery name="qList" datasource="#variables.dsn#">
			SELECT 	<cfif structKeyExists(arguments,"maxrows") AND arguments.maxrows GT 0>TOP #arguments.maxrows#</cfif>
					<<cfloop from="1" to="$$ArrayLen(aFields)$$" index="i">>[$$tableName$$].[$$aFields[i].name$$] AS [$$aFields[i].alias$$]<<cfif i NEQ ArrayLen(aFields) OR ArrayLen(aJoinedFields) GT 0 >>,$$chr(10)$$<</cfif>>					<</cfloop>>
					<<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>[$$aJoinedFields[i].table$$].[$$aJoinedFields[i].name$$] AS [$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$]<<cfif i NEQ ArrayLen(aJoinedFields)>>,$$chr(10)$$<</cfif>>					<</cfloop>>
		...
		<<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>
			<cfif structKeyExists(arguments,"$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$") and len(arguments.$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$)>
				AND	[$$aJoinedFields[i].table$$].[$$aJoinedFields[i].name$$] = <cfqueryparam value="#arguments.$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$#" CFSQLType="$$aJoinedFields[i].SQLType$$" />
			</cfif><</cfloop>>
