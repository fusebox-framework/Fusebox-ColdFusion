		...
		<cfloop list="#fieldlist#" index="thisField">
			<cfswitch expression="#thisField#">
				...
				<cfcase value="$$lFirst(aFields[i].alias)$$">
					...
					<cfif mode is "edit">
					<tr>
						<th width="80" align="left" <cfif ListFindNocase(highlightfields,'$$lFirst(aFields[i].alias)$$')>class="#variables.highlightThClass#"<cfelse>class="#variables.standardThClass#"</cfif> >
							$$aFields[i].label$$
						</th>
						<td>
					...
					<tr>
						<th width="80" align="left" <cfif ListFindNocase(highlightfields,'$$lFirst(aFields[i].alias)$$')>class="#variables.highlightThClass#"<cfelse>class="#variables.standardThClass#"</cfif> >
							<<cfif ListLast(aFields[i].label," ") IS "Id" AND aFields[i].parent IS NOT "">>$$Left(aFields[i].label,Len(aFields[i].label)-3)$$<<cfelse>>$$aFields[i].label$$<</cfif>>
						</th>
						<td>
					...
					<tr>
						...
						<td>
							<cfif o$$uFirst(objectName)$$.get$$uFirst(aFields[i].alias)$$() IS "1-January-0100"><cfset displayValue = ""><cfelse>
							<cftry><cfset displayValue = $$Format("o$$uFirst(objectName)$$.get$$uFirst(aFields[i].alias)$$()","$$aFields[i].format$$")$$><cfcatch><cfset displayValue = ""></cfcatch></cftry></cfif>
					...
					<<cfelseif aFields[i].format IS "Currency">>
					<!--- Currency --->
					<tr>
						<th width="80" align="left" <cfif ListFindNocase(highlightfields,'$$lFirst(aFields[i].alias)$$')>class="#variables.highlightThClass#"<cfelse>class="#variables.standardThClass#"</cfif> >
							$$aFields[i].label$$
						...
						<td>
							<input type="Text" name="$$lFirst(aFields[i].alias)$$" id="$$lFirst(aFields[i].alias)$$" size="$$aFields[i].size$$" <<cfif structKeyExists(aFields[i],"maxlength")>>maxlength="$$aFields[i].maxlength$$"<</cfif>> value="<cftry>#NumberFormat(o$$uFirst(objectName)$$.get$$uFirst(aFields[i].alias)$$(),"9.99")#<cfcatch>#o$$uFirst(objectName)$$.get$$uFirst(aFields[i].alias)$$()#</cfcatch></cftry>" />
						</td>
					</tr>
					<<cfelse>>
					<!--- default text box --->
