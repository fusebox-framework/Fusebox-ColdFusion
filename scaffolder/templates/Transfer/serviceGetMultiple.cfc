...
<<cfoutput>>
	<cffunction name="get$$objectName$$s" access="public" output="false" returntype="query">
	  <<cfloop from="1" to="$$ArrayLen(aFields)$$" index="i">>
		<cfargument name="$$aFields[i].alias$$" type="<<cfif aFields[i].type IS "boolean">>Any<<cfelse>>$$aFields[i].type$$<</cfif>>" required="false" <<cfif aFields[i].alias IS "active">>default="true"<</cfif>> /><</cfloop>>
	  <<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>
		<cfargument name="$$aJoinedFields[i].table$$$$aJoinedFields[i].alias$$" type="<<cfif aJoinedFields[i].type IS "boolean">>Any<<cfelse>>$$aJoinedFields[i].type$$<</cfif>>" required="false" <<cfif aJoinedFields[i].alias IS "active">>default="true"<</cfif>> /><</cfloop>>
