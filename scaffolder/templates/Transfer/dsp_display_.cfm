...
<<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>
	<<cfset lParentFields = ListAppend(lParentFields,aJoinedFields[i].table & aJoinedFields[i].alias)>>
<</cfloop>>
...
<cfif isDefined("o$$uFirst(objectName)$$")>
	...
	<<cfset lFields = oMetaData.getFieldListFromXML(aManyToOne[thisParent].alias)>><cfset lFields = "$$lFields$$">
	<cfloop list="#lFields#" index="thisField">
		<cfset setvariable("variables.$$lFirst(aManyToOne[thisParent].alias)$$#thisField#", Evaluate("o$$aManyToOne[thisParent].alias$$.get#thisField#()"))>
	</cfloop>
	<</cfloop>>
	<<cfset lFields = oMetaData.getFieldListFromXML(objectName)>><cfloop list="$$lFields$$" index="thisField">
				...
				<tr>
				  ...
				  <cfelse>
					<th align="left" class="standard">
						...
						<<cfif ListLast(aFields[i].label," ") IS "Id" AND structKeyExists(aFields[i],"parent")>>$$Left(aFields[i].label,Len(aFields[i].label)-3)$$<<cfelse>>$$aFields[i].label$$<</cfif>>
					</th>
					<td>
						<<cfif aFields[i].formType IS "Password">>****
						<<cfelseif ListLast(aFields[i].label," ") IS "Id" AND structKeyExists(aFields[i],"parent") AND ListFindNoCase(lParentFields,"$$aFields[i].parent$$Name")>>#variables.$$aFields[i].parent$$Name#&nbsp;
						<<cfelseif ListLast(aFields[i].label," ") IS "Id" AND structKeyExists(aFields[i],"parent") AND ListFindNoCase(lParentFields,"$$aFields[i].parent$$Title")>>#variables.$$aFields[i].parent$$Title#&nbsp;
						<<cfelse>><cftry>#$$Format("variables.$$lFirst(aFields[i].alias)$$","$$aFields[i].format$$")$$#<cfcatch>#variables.$$lFirst(aFields[i].alias)$$#</cfcatch></cftry><</cfif>>
					</td>
				</tr>
			</cfcase><</cfloop>>
			<<!--- <<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>
